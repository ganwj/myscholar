// @ts-nocheck
import { hasClassName, addClassName, removeClassName, capitalize } from './util';
...
export function createDropdown(
	...
	const makeWidget = panel({
		...
		templates: {
			header: (options, { html }) => {
				...
				return html`<button type="button" class="${cx(...classNames)}">${text}</button>`;
			},
			footer(options, { html }) {
	...
	return (widgetParams) => {
		...
		return {
			...widget,
			init: (options) => {
				const rootElem = document.querySelector(widgetParams.container).querySelector('.ais-Panel');
				const headerElem = rootElem.querySelector('.ais-Panel-header');
				const closeButtonElem = rootElem.querySelector('.' + CLASS_CLOSE_BUTTON);
